...
class BookmarksController < ApplicationController
  ...
  def fetch_genres_by_mood
    if @mood_name.present?
      mood = Mood.find_by('LOWER(name) = ?', @mood_name)
      @genres_by_mood = mood ? mood.genres.where(genre_format: @content_format) : []
    else
      @genres_by_mood = Genre.where(genre_format: @content_format).sample(1)
    end
  end
end
